---
title: "Diamond Market Modeling Project"
author: "Ivan Gladnikov"
format: pdf
editor: 
  markdown: 
    wrap: 72
...
The sample contains the following variables:

**Response Variable:**

-   price: Price of the diamond in US dollars ranging from \$361 to
    \$18806. This is the target variable for prediction. It's very skewed, with a few high outliers affecting the statistics.
...
-   z: Depth of the diamond in millimeters ranging from 0mm to 5.41mm.
    (A measurement of 0mm is impossible which means that I have to
...
```{r Histograms, fig.height=8, fig.width=10}
...
```{r Bar Plots, fig.height=4, fig.width=10}
...
## Choosing variables and determine correlation
...
3.  Most diamonds in the sample have Ideal or Premium cuts, G or E
    colors, and SI1 or VS2 clarity grade which shows that the sample
...
## Starting with one predictor and one response

```{r}
slr_model <- lm(price ~ carat, data = diamonds_sample)
```

##  Running the model and examining summary statistics
...
## Testing assumptions and applying transformations
...
To correct these violations and validate the model's assumptions, I
applied a **log-log transformation** to both the response variable (price)
and the predictor (carat).

### Summary of Transformed Variables and Changes

The model is refitted as $\log(\text{price}) \sim \log(\text{carat})$.

```{r Summary Transformed}
...
## Adding other variables and assessing improvement
...
```{r Best Model}
# Based on Part 2, the best model includes all variables with log(price)
summary(best_model)
```
...
**Observations:** - Full model Adjusted R²: 0.9841, explaining 98.41% of
...
a sample of 1,000 diamonds. My findings include realizing and proving
...
3.  **Multicollinearity**:
    ...
    -   I decided to remove x, y, and z to retain log_carat as the main
        size measure.
